...
更新源就是社区下载区里「社区客户端APP」那张卡片（`157664f3-7ae8-4509-b032-ebf1437b11ce`），
卡片树是「客户端 → 平台 → 版本 → 各安装包」：版本卡片的标题就是版本号，文件卡片是
`redirectUrl` 指向真实下载地址。客户端拉 `/downloads/cards`，按当前平台找到版本号最大的
那张卡片，与本机版本逐段比较。

- **自动**：启动时和回到前台时各检查一次，6 小时内只真正请求一次；发现新版本、且用户没点过
  「不再提醒」才弹窗。选「下载安装」会在应用内下载，再交给系统安装器打开；系统里没有能打开
  该安装包的应用时退回浏览器下载。开发构建拿不到可比版本号，不会自动弹窗。
- **手动**：「我的 → 关于 → 检查更新」，每次都真的请求，并直接给出结果（有新版 / 已是最新 /
  当前平台不支持 / 请求失败可重试）。

Web 端不参与自动更新（刷新页面即最新）。macOS、iOS 只有在下载区里建好对应平台卡片后才会
匹配到；平台卡片不存在时提示「无法自动检查」，不会报错。

Android 走「应用内下载 + 系统安装器」需要 `REQUEST_INSTALL_PACKAGES` 权限（已加入 manifest）；
不通过 Google Play 分发时没有额外影响，日后上架需按 Play 政策说明该权限用途。
...
应用名称为「晏阳社区」，各平台图标统一由 `assets/app_icon.png`（256×256 官方图标：圆角蓝底 #5DA4FA + 白色标识）生成；重新生成可运行 `python3 tool/generate_icons.py`（需要 Pillow）。iOS/macOS 的 1024、macOS/web 的 512、Android 自适应前景比主图更大，脚本按主图里量出的圆角半径与标识位置重建，避免把 256 放大导致细线发虚。Android 自适应图标的底色来自生成的 `res/values/colors.xml`。
